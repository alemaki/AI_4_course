...
policy_net = DQN(input_shape, n_actions).to('cuda')
target_net = DQN(input_shape, n_actions).to('cuda')
...
for episode in range(10000):
    ...
    for step in range(1500):
        ...
        if len(memory) > BATCH_SIZE:
            ...
            states = torch.FloatTensor(np.array(states)).to('cuda')
            actions = torch.LongTensor(actions).to('cuda')
            rewards = torch.FloatTensor(rewards).to('cuda')
            next_states = torch.FloatTensor(np.array(next_states)).to('cuda')
