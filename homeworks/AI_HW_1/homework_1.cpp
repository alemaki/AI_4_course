...
std::unordered_set<Board, BoardHash> visited_boards;
...
void create_fhg_score(Node* node)
{
    if (!node)
    {
        return;
...
bool check_valid_board(const Board& board)
{
    if (visited_boards.find(board) != visited_boards.end())
    {
        return false;
    }
    visited_boards.insert(board);
    return true;
}

Node* create_node_with_move(const Board& board, Move current_move, Node* parent)
{
    ...
    if(!check_valid_board(board_copy))
    {
        return nullptr;
    }
...
void generate_children(Node* root)
{
    create_node_with_move(root->current_board, Move::LEFT, root); // pushes child on it's own
    create_node_with_move(root->current_board, Move::RIGHT, root);
    create_node_with_move(root->current_board, Move::UP, root);
    create_node_with_move(root->current_board, Move::DOWN, root);

    root->generated_children = true;
}


int64_t IDA_search(Node* node, int64_t threshold, Node*& solution)
{
    /*if (!node)
    {
    ...
    {
        int64_t result = IDA_search(child, threshold, solution);
        if (solution)
        {
            return 0;
        }
...
Node* IDA_star(Node* root)
{
    ...
    while (true) //bad idea, maybe threshold < 10^5?
    {
        ...
        int64_t result = IDA_search(root, threshold, solution);
        if (solution)
        {
            return solution;
        }
...
int main(int argc, char* argv[])
{
    ...
    {
        steps++;
        moves.push(solution->current_move);
